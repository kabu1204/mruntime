...
bool has_complete_event_named(const std::vector<mruntime::TraceEvent>& events, const char* name) {
    for (const auto& e : events) {
        if (e.type == mruntime::TraceEventType::Complete && e.name != nullptr &&
...
int main() {
    try {
        ...
        {
            TRACE_SCOPE_CAT("vector_add", "test");
            mruntime::vulkan::VkDispatchTraceInfo trace_info;
        ...
        const auto& events = mruntime::TraceCollector::instance().events();
        if (!has_complete_event_named(events, "vk.dispatch") ||
            !has_complete_event_named(events, "vk.launch") ||
            !has_complete_event_named(events, "vk.wait")) {
            throw std::runtime_error("Missing expected Vulkan timing trace events");
        }

        if (ctx.timestamp_query_pool() != VK_NULL_HANDLE) {
            if (!has_complete_event_named(events, "vk.kernel")) {
                throw std::runtime_error("Expected vk.kernel event when timestamp query pool is available");
            }
        }

        std::cout << "vulkan_trace_timing_test PASSED\n";
        return 0;
    } catch (const mruntime::vulkan::VulkanError& e) {
        if (e.result() == VK_ERROR_INCOMPATIBLE_DRIVER) {
            std::cout << "vulkan_trace_timing_test SKIPPED: Vulkan not supported on this machine\n";
            return 77;
        }
        std::cerr << "vulkan_trace_timing_test FAILED: " << e.what() << "\n";
        return 1;
